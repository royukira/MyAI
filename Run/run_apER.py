    ...
    for i_episode in range(20):
        ...
        while True:
            ...
            if (total_steps > init_capacity) and (is_learn == 0):
                # Learning part
                print("--> Step {0} : Adaptive - Prioritized Experience Replay...".format(total_steps))
                _, gd = RL.prioritized_Learn()
                gradient.append(gd)
...
if __name__ == '__main__':
    ...
    rl = apER.apER_train(numState=N_STATES,actions=ACTIONS,learnRate=ALPHA,
                         discount=EPSILON,greedy=GAMMA,init_memorySize=memorysize,
                         batchSize=batchSize,k_old=20)

    step, gd= train_with_play(RL=rl, numState=N_STATES, init_capacity=memorysize)

    plt.figure(2)
    plt.plot(step[0, :], step[1, :], c='r', label='Linear Q-learning with apER')
    plt.legend(loc='best')
